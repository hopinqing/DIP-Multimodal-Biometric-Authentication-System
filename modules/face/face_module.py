...
import time
import winsound
import argparse # <-- Added so it can read command line arguments
...
# Note: EMBEDDING_FILE is now dynamically generated in the functions below using user_id!
...
def open_camera():
    # 1. Initialize camera (0 is the only active camera if built-in is disabled)
    cap = cv2.VideoCapture(0)
    cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    
    # 2. FORCE downscale the resolution to make it lightning fast for external webcams
    ...
    while True:
        ...
        if result.facial_transformation_matrixes and result.face_landmarks:
            ...
            if waiting_for_center:
                ...
                cv2.putText(frame, f"Step {step_index+1}/{LIVENESS_STEPS}: TURN {direction} (Slowly)",
...
def run_face_enrollment(user_id="DemoUser"):
    print(f"[INFO] Preparing camera for Face Enrollment for user: {user_id}...")
    
    user_embedding_file = f"{user_id}_face_embeddings.npy"
    
    cap = open_camera()
    window = "Face Enrollment"
    
    # 3. FORCE the window to be resizable and larger
    ...
    try:
        ...
        if embeddings_array is not None:
            ...
            np.save(user_embedding_file, np.array([master_embedding]))
            print(f"[SUCCESS] Face enrolled successfully for {user_id}.")
            return True
        else:
...
def run_face_verification(user_id="DemoUser"):
    user_embedding_file = f"{user_id}_face_embeddings.npy"

    if not os.path.exists(user_embedding_file):
        print(f"[ERROR] No enrolled face template found for {user_id}.")
        return False

    print(f"[INFO] Preparing camera for Face Authentication for user: {user_id}...")
    cap = open_camera()
    ...
    try:
        ...
        stored_embeddings = np.load(user_embedding_file)
...
if __name__ == "__main__":
    ...
    if args.enroll:
        success = run_face_enrollment(args.user)
        sys.exit(0 if success else 1)
    else:
        success = run_face_verification(args.user)
        sys.exit(0 if success else 1)